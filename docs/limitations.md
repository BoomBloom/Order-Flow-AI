...
| --- | --- | --- | --- |
| V1 | Vendor identity, tiers, cost, and history depth | Phase 1 | Limited Databento `GLBX.MDP3`/`NQU6` sample and provisional 20-weekday preflight observed 2026-09-06; no source/tier selected. See `vendor_capability_matrix.md`. |
...
| V7 | Storage volume per instrument-day at each data tier | Scaling beyond one instrument | One Databento MBP-1 UTC-day sample is 190,231,112 compressed bytes; a 20-weekday quote is 15,801,014,560 billable bytes. Neither is a local-storage scaling estimate. |
