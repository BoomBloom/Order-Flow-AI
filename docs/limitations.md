...
| V9 | **Rithmic developer-API access path and cost.** Rithmic publishes R\|API+ (C++/.NET), R\|Protocol (wire spec), and R\|Diamond. Secondary sources describe the route as: request a dev kit, build against their test system, pass a conformance review, then receive production credentials — typically sponsored through the clearing broker. Fees, timeline, and whether an existing R\|Trader Pro market-data entitlement carries over are unknown. | Whether a first-party live adapter (`architecture.md` §12) is reachable, or whether live capture must run through a third-party front-end | Secondary sources only (vendor/broker marketing pages), 2026-09-04 — **not** primary documentation; does not satisfy §6 |
